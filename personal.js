    ...
    list.innerHTML = personas.map(persona => `
        <div class="persona-card ${persona.isActive ? 'active' : ''}">
            ...
            <div class="persona-actions">
                ${!persona.isActive ? 
                    `<button class="btn-activate" onclick="activatePersona(${personas.indexOf(persona)})">激活</button>` :
                    `<span class="active-badge">当前使用</span>`}
                <button class="btn-edit" onclick="editPersona(${personas.indexOf(persona)})">编辑</button>
                <button class="btn-delete" onclick="deletePersona(${personas.indexOf(persona)})">删除</button>
            </div>
        </div>
    `).join('');
}
...
function showCreatePersona() {
    ...
    document.getElementById('persona-list').style.display = 'none';
...
function deletePersona(index) {
    ...
    if (persona.isActive && personas.length > 1) {
        ...
        personas[0].isActive = true;
        applyPersonaToAI(personas[0]);
    }
...
function changeCalendarMonth(direction) {
    ...
    document.getElementById('calendarMonth').textContent = 
        `${currentCalendarMonth.getFullYear()}年${monthNames[currentCalendarMonth.getMonth()]}`;
